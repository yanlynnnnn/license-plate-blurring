...
def copy_file(src_path, dst_path):
    ...
    data_bytes = device.Pull(src_path)
...
def copy():
    ...
    files = device.List(base_dir_path)
    for file in files:
        file_name = getattr(file, "filename").decode("utf-8")

        if "." in file_name:
            # file: jpg / mp4 / srt
            copy_file(base_dir_path + file_name, data_dir_path + "/" + file_name)
        else:
            # dir
            Path(data_dir_path + "/" + file_name).mkdir(exist_ok=True)

            for sub_file in device.List(base_dir_path + file_name):
